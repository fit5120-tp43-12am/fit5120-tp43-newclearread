# Central-Brain Review: Worker 006 Full Training Candidate A
...
Worker 006 successfully completed Candidate A full training:
...
C:\Users\Aufb\Desktop\fit5120\iteration1\training\scripts\train_full_qlora.py
C:\Users\Aufb\Desktop\fit5120\iteration1\training\scripts\run_inference_check.py
...
C:\Users\Aufb\Desktop\fit5120\iteration1\training\logs\decisions\worker_006_train_candidate_a_3epoch.md
```

Key Worker 006 results:
...
Central-brain verified local split hashes:
...
Central-brain verified local line counts:
...
Central-brain verified the Candidate A adapter directory exists:
...
Central-brain reran compile verification in WSL env `clearread-llama-lora`:
...
Central-brain independently reran validation inference against Candidate A:

```bash
python scripts/run_inference_check.py --config configs/train_llama31_8b_qlora_candidate_a.yaml --adapter-path models/adapters/full_candidate_a_3epoch --data-path data/splits/val.jsonl --num-examples 5
```
...
Central-brain searched Candidate A config/script/log/report for test-set usage. The only hit was the report instruction saying not to use the test set until central-brain selection. No `test.jsonl` path was used.
...
- latest commits:
  - `f97b063 docs(ai): finalize worker 006 git record`
  - `7a637a8 docs(ai): record worker 006 training result`
...
The next worker should perform a validation-only Candidate A quality audit before the test set is touched. This audit should generate validation predictions, run deterministic format/readability/encoding checks, and do a stratified manual-quality review across domains and length buckets. Central brain will then decide whether to:
