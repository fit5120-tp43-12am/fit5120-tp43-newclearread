  ...
  if ! run_logged "${stage_label}_judge_api_${model_key}" \
    ...
    if ! run_logged "${stage_label}_judge_api_${model_key}_retry_failures" \
      python scripts/run_judge_api.py \
        ...
        --model-key "$model_key" \
        --retry-failures; then
      echo "[postprocess-run] warning: judge retry returned nonzero for ${model_key}; continuing so later epochs are still recorded." >&2
...
echo "[postprocess-run] completed for ${candidate_key} / ${run_id}"
